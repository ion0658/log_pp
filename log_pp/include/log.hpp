...
template <typename CharT>
/**
 * @brief Overload of `log()` using global logger without message.
 *
 * Prefer using the `LOG_PP_*` macros for automatic source location capture.
 *
 * Example:
 * @code
 * LOG_INFO(log_pp::basic_target_t<char>{"api"},
 *          {{"key", "value"}, {"attempt", 3}});
 * @endcode
 *
 * @param level Severity level.
...
void log(Level level,
         ...
         basic_target_t<CharT> target,
         std::initializer_list<BasicKV<CharT>> kvs) {
    log(level, module, logger<CharT>(), target, kvs);
}
 ...
 * Example:
 * @code
 * LOG_INFO({"api"}, {{"key", "value"}, {"attempt", 3}});
 * @endcode
 *
 * @param level Severity level.
 * @param module Source location metadata.
...
void log(Level level,
         ...
         std::initializer_list<BasicKV<CharT>> kvs) {
    if (target.size() > 0) {
        log(level, module, logger<CharT>(),
            basic_target_t<CharT>{*target.begin()}, kvs);
    } else {
        log(level, module, logger<CharT>(), basic_target_t<CharT>{}, kvs);
    }
}

template <typename CharT>
/**
 * @brief Overload of `log()` using global logger without message and target.
 *
 * Prefer using the `LOG_PP_*` macros for automatic source location capture.
 *
 * Example:
 * @code
 * LOG_INFO(std::initializer_list<log_pp::BasicKV<char>>{
 *                                                          {"key", "value"},
 *                                                          {"attempt", 3}
 *                                                      }
 *          );
