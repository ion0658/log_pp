...
struct BasicKV {
   ...
   public:
    ...
    std::basic_string_view<CharT> get_key_str() const noexcept { return key; }
    /** @brief Returns the formatted value. @return Formatted value text. */
    std::basic_string<CharT> get_value_string() const noexcept {
        return value_formatter();
    }
};

template <typename CharT, typename T>
BasicKV(std::basic_string_view<CharT>, T&&) -> BasicKV<CharT>;

template <typename CharT, typename T>
BasicKV(std::basic_string_view<CharT>, T&&, std::basic_string_view<CharT>)
    -> BasicKV<CharT>;

template <typename CharT, typename T>
BasicKV(const CharT*, T&&) -> BasicKV<CharT>;

template <typename CharT, typename T>
BasicKV(const CharT*, T&&, const CharT*) -> BasicKV<CharT>;
