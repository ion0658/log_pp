...
template <>
struct CaptureLogger<wchar_t> : public log_pp::BasicLogger<wchar_t> {
...
TEST(log_pp_kv, kv_only_with_explicit_logger_without_target) {
    static CaptureLogger<char> logger;

    log_pp::log<char>(log_pp::Level::Info, std::source_location::current(),
                      logger, {{"ok", true}, {"elapsed_ms", 12.0}});

    EXPECT_EQ("", logger.last_target);
    EXPECT_EQ("", logger.last_message);
    EXPECT_EQ("ok=true;elapsed_ms=12", logger.last_kv_dump);
}

TEST(log_pp_kv, kv_only_with_explicit_logger_with_target) {
    static CaptureLogger<char> logger;

    log_pp::log<char>(log_pp::Level::Info, std::source_location::current(),
                      logger, {"explicit"}, {{"status", "ok"}, {"attempt", 2}});

    EXPECT_EQ("explicit", logger.last_target);
    EXPECT_EQ("", logger.last_message);
    EXPECT_EQ("status=ok;attempt=2", logger.last_kv_dump);
}
...
TEST(log_pp_kv, basic_kv_direct_construction_with_format_view) {
    ...
    EXPECT_EQ("id", kv.get_key_str());
    EXPECT_EQ("0x1A", kv.get_value_string());
}
...
TEST(log_pp_kv, kv_keeps_lvalue_value_by_reference_when_possible) {
    ...
    user = "bob";
    EXPECT_EQ("bob", kv.get_value_string());
}

TEST(log_pp_kv, kv_keeps_rvalue_value_by_ownership) {
    auto kv = log_pp::KV{"user", std::string{"alice"}};

    EXPECT_EQ("alice", kv.get_value_string());
}

TEST(log_pp_kv, wchar_kv_only_with_explicit_logger_without_target) {
    static CaptureLogger<wchar_t> logger;

    log_pp::log<wchar_t>(log_pp::Level::Info, std::source_location::current(),
                         logger, {{L"ok", true}, {L"attempt", 2}});

    EXPECT_EQ(L"", logger.last_target);
    EXPECT_EQ(L"", logger.last_message);
    EXPECT_EQ(L"ok=true;attempt=2", logger.last_kv_dump);
}

TEST(log_pp_kv, wchar_kv_only_with_explicit_logger_with_target) {
    static CaptureLogger<wchar_t> logger;

    log_pp::log<wchar_t>(log_pp::Level::Info, std::source_location::current(),
                         logger, {L"wide_target"},
                         {{L"status", L"ok"}, {L"attempt", 2}});

    EXPECT_EQ(L"wide_target", logger.last_target);
    EXPECT_EQ(L"", logger.last_message);
    EXPECT_EQ(L"status=ok;attempt=2", logger.last_kv_dump);
}
